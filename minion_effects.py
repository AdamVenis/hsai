...
def cairne_bloodhoof(game, trigger, id):
   if trigger[0] == 'kill_minion' and trigger[1] == id:
      game.action_queue.append((actions.spawn, (game, game.minion_pool[id].owner, utils.get_card('Baine Bloodhoof'))))
      return True
...
def harvest_golem(game, trigger, id):
   if trigger[0] == 'kill_minion' and trigger[1] == id:
      game.action_queue.append((actions.spawn, (game, game.minion_pool[id].owner, utils.get_card('Damaged Golem'))))
      return True
...
def healing_totem(game, trigger, id):
   if trigger[0] == 'end_turn' and game.minion_pool[id].owner == trigger[1]:
      for minion in game.minion_pool[id].owner.board[1:]:
         game.action_queue.append((actions.heal, (game, minion.minion_id, 1)))
